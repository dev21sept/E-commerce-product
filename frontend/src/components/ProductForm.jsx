...
// --- NEW SEARCHABLE CATEGORY COMPONENT (EBAY CLONE) ---
                        ...
                        <div className="p-6 bg-gray-50 border-b border-gray-100">
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    autoFocus
                                    type="text"
                                    placeholder="Type category (e.g. T-shirts, Shoes, Watches)..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="w-full pl-12 pr-6 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-bold outline-none focus:border-blue-600 shadow-sm"
                                />
                        ...
                        <div className="max-h-[400px] overflow-y-auto scrollbar-thin">
                            {suggestions.length > 0 ? (
                                suggestions.map((s, i) => (
                                    <div 
                                        key={i} 
                                        onClick={() => { onChange(s.fullName, s.id); setIsOpen(false); }} 
                                        className="px-6 py-5 border-b border-gray-50 hover:bg-blue-600 group cursor-pointer transition-all"
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-full bg-blue-50 group-hover:bg-blue-500/20 flex items-center justify-center shrink-0">
                                                <Layers className="w-5 h-5 text-blue-600 group-hover:text-white" />
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-bold text-gray-900 group-hover:text-white leading-tight">{s.fullName}</span>
                                                <span className="text-[10px] font-black group-hover:text-blue-100 text-gray-400 uppercase tracking-widest mt-1">ID: {s.id}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
                                ...
                                <div className="p-12 text-center">
                                    <p className="text-sm font-bold text-gray-400 italic">{searchTerm.length > 2 ? 'Searching for categories...' : 'Type at least 3 characters to search'}</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="lg:col-span-8 space-y-12">
                    <div className="space-y-8">
                        <textarea
                            ...
                            rows="2"
                            className="w-full text-5xl font-black text-gray-900 border-none outline-none p-0 resize-none leading-tight tracking-tighter bg-transparent"
                        ...
                        <div className="grid grid-cols-2 gap-8 pt-8 border-t border-gray-100">
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-1 block">Product Category</label>
                    ...
                    <div className="pt-12 border-t border-gray-100">
                        <div className="flex items-center justify-between mb-10">
                            <div><h3 className="text-2xl font-black text-gray-900 tracking-tight">Required & Suggested Specifics</h3><p className="text-xs text-gray-400 font-medium">Official eBay taxonomy data fetched via API.</p></div>
                    ...
                    <div className="pt-12 border-t border-gray-100">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-6">
                            {formData.images.map((img, idx) => (
                                <div key={idx} className="relative aspect-square rounded-[30px] overflow-hidden border border-gray-50 group">
                            ...
                            <label className="aspect-square rounded-[30px] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 transition-all">
                                <Upload className="w-6 h-6 text-gray-300" /><input type="file" multiple className="hidden" onChange={handleFileUpload} />
                            </label>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-4 space-y-10">
                    <div className="sticky top-10 space-y-10">
                        <div className="bg-white rounded-[40px] border border-gray-100 p-8 shadow-sm space-y-4">
                            <button type="submit" disabled={isFetching} className="w-full py-6 bg-gray-900 text-white rounded-full font-black text-sm uppercase tracking-widest flex items-center justify-center gap-4">
                                {isFetching ? <Loader2 className="w-5 h-5 animate-spin"/> : <Save className="w-5 h-5"/>} {initialData ? 'Update Record' : 'Save To Product List'}
                            </button>
                            <button type="button" onClick={() => { window.postMessage({ type: "EbayAutoLister_SendData", payload: formData }, "*"); alert("DATA SYNCED!"); }} className="w-full py-6 bg-[#0064D2] text-white rounded-full font-black text-sm uppercase tracking-widest flex items-center justify-center gap-4">
                                <ExternalLink className="w-5 h-5" /> Push To eBay Live
                            </button>
                        </div>
                    </div>
                </div>
